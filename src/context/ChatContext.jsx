...
export function ChatProvider({ children }) {
  ...
  const fetchMessages = useCallback(async (conversationId) => {
      ...
      .select(`
        *,
        reply_to:reply_to_id (
          id, content, nonce, message_type, sender_id
        ),
        message_reactions (
          id, emoji, user_id
        )
    ...
    // Collect all referenced user IDs for a single batch profile query
    const userIds = new Set()
    rows.forEach(m => {
      if (m.sender_id) userIds.add(m.sender_id)
      if (m.reply_to?.sender_id) userIds.add(m.reply_to.sender_id)
      ;(m.message_reactions || []).forEach(r => { if (r.user_id) userIds.add(r.user_id) })
    })
    ...
    const populatedData = rows.map(m => ({
      ...m,
      profiles: profileMap[m.sender_id] || null,
      reply_to: m.reply_to ? {
        ...m.reply_to,
        profiles: profileMap[m.reply_to.sender_id] || null
      } : null,
    ...
    const decrypted = await Promise.all(populatedData.map(row => decryptMessageRow(row, conversationId)))
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        const { data: msgData } = await supabase
          .from('messages')
          .select(`*, reply_to:reply_to_id (id, content, nonce, message_type, sender_id), message_reactions (id, emoji, user_id)`)
