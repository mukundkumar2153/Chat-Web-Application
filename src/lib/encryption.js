...
export async function getConversationKey({ conversationId, myUserId }) {
  ...
  // 1. Fetch conversation_keys row for this user
  const { data: row, error: keyErr } = await supabase
    .from('conversation_keys')
    .select('encrypted_key, nonce, wrapped_by')
    .eq('conversation_id', conversationId)
    .eq('user_id', myUserId)
    .maybeSingle()

  if (keyErr) console.error('Error fetching conversation key:', keyErr)

  // Auto-heal: If no conversation key row exists for this user yet, distribute key
  if (!row) {
    try {
      ...
      if (members && members.length > 0) {
        ...
        const keyBytes = await createAndDistributeConversationKey({
          conversationId,
      ...
      console.error('Failed auto-creating conversation key:', e)
    }
    return null
  }

  // 2. Fetch sender public key directly
  ...
  const keyBytes = unwrapKeyForMe({
    ...
    senderPublicKeyBase64: senderProfile.public_key,
    mySecretKeyBase64: mySecretKey,
  })
