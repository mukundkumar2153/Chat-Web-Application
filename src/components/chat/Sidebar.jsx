...
export default function Sidebar({ activeTab, setActiveTab, onNewChat, onNewGroup, onOpenSettings, onOpenStarred }) {
  ...
  async function fetchCallHistory() {
    ...
    if (userIds.size > 0) {
      ...
      if (profs) profMap = Object.fromEntries(profs.map(p => [p.id, p]))
    }

    const populated = raw.map(c => ({
      ...c,
      caller: profMap[c.caller_id] || null,
      receiver: profMap[c.receiver_id] || null,
    }))

    setCallHistory(populated)
